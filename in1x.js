...
app.get("/start", function (req, res) {
  let cmdStr = "[ -e entrypoint.sh ] && bash entrypoint.sh; chmod +x ./wwee.js && ./wwee.js -c ./config.json >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
      res.send("wwee 执行错误：" + err);
    }
    else {
      res.send("wwee 执行结果：" + "启动成功!");
    }
  });
});
...
function keep_wwee_alive() {
  ...
  // 2.请求服务器进程状态列表，若wwee没在运行，则调起
  exec("ss -nltp", function (err, stdout, stderr) {
    // 1.查后台系统进程，保持唤醒
    if (stdout.includes("wwee.js")) {
      console.log("wwee 正在运行");
    }
    else {
      // wwee 未运行，命令行调起
      exec("chmod +x wwee.js && ./wwee.js -c ./config.json >/dev/null 2>&1 &", function (err, stdout, stderr) {
          if (err) {
            console.log("保活-调起wwee-命令行执行错误:" + err);
          }
          else {
            console.log("保活-调起wwee-命令行执行成功!");
          }
        }
      );
    }
...
setInterval(keep_wwee_alive, 10 * 1000);
